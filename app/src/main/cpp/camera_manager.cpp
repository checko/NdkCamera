...
NDKCamera::NDKCamera()
  ...
  // Pick up a configured facing camera to preview
  EnumerateCamera(ACAMERA_LENS_FACING_FRONT);
...
bool NDKCamera::MatchCaptureSizeRequest(int32_t requestWidth,
  ...
  LOGI(" --> Image Width %d", resView->width);
  LOGI(" --> Image Height %d", resView->height);
...
AImageReader* NDKCamera::createJpegReader()
{
